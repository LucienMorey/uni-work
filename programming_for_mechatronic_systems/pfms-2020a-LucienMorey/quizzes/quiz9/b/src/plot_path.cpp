    ...
    while (ros::ok())
    {
      ...
      if (imageOK)
      {
        if (pathArrayBuffer_.received)
        {
          ...
          //! @todo - Q4
          //!
          //! Convert the global path to pixel coordinates
          //! This will be reverse of the code supplied in gen_path CallBackFunc

          //! @todo - Q5
          //! Draw the checkpoints on the image
          //!  Use the circle to draw the pose
          //! cv::circle(rgbImage, cv::Point((xD), (yD)), 3, CV_RGB(255,0,0),-1);

          for (auto pose : pathArrayBuffer_.poses.poses)
          {
            int pixel_x = (pose.position.x) / pathArrayBuffer_.resolution +
                          (pathArrayBuffer_.mapSize / pathArrayBuffer_.resolution) / 2;
            int pixel_y = (pose.position.y) / -pathArrayBuffer_.resolution +
                          (pathArrayBuffer_.mapSize / pathArrayBuffer_.resolution) / 2;

            if ((pixel_x <= pathArrayBuffer_.mapSize / pathArrayBuffer_.resolution) && (pixel_x >= 0) &&
                (pixel_y <= pathArrayBuffer_.mapSize / pathArrayBuffer_.resolution) && (pixel_y >= 0))
            {
              cv::circle(rgbImage, cv::Point((pixel_x), (pixel_y)), 3, CV_RGB(255, 0, 0), -1);
            }
          }
          ...
          cv::putText(rgbImage, "Received Path.", cv::Point(30, 30), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8,
