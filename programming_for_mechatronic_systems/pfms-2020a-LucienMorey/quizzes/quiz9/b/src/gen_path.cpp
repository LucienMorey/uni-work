...
void CallBackFunc(int event, int x, int y, int flags, void* param)
{
  ...
  if (event == cv::EVENT_LBUTTONDOWN)
  {
    ...
    // The centre of the OgMap is (0,0) in [m]
    double centre = (poseArrayBuffer->mapSize / poseArrayBuffer->resolution) / 2;
    ...
    pose.position.x = (double(x) - (centre)) * poseArrayBuffer->resolution + poseArrayBuffer->vehPose.position.x;
    pose.position.y = -(double(y) - (centre)) * poseArrayBuffer->resolution + poseArrayBuffer->vehPose.position.y;
    tf2::Quaternion q;
    q.setRPY(0, 0, 0);
    q.normalize();

    // ROS_INFO_STREAM(q.getW());
    pose.orientation = tf2::toMsg(q);

    poseArrayBuffer->poses.poses.push_back(pose);
    if (poseArrayBuffer->poses.poses.size() > 1)
    {
      double heading_to_target_error = atan2((*(poseArrayBuffer->poses.poses.end() - 1)).position.y -
                                                 (*(poseArrayBuffer->poses.poses.end() - 2)).position.y,
                                             (*(poseArrayBuffer->poses.poses.end() - 1)).position.x -
                                                 (*(poseArrayBuffer->poses.poses.end() - 2)).position.x);
      q.setRPY(0, 0, heading_to_target_error);
      (*(poseArrayBuffer->poses.poses.end() - 2)).orientation = tf2::toMsg(q);
    }
    poseArrayBuffer->mtx.unlock();

    std::cout << "centre " << centre << std::endl;
    std::cout << "map resolution " << poseArrayBuffer->resolution << std::endl;
    std::cout << "vehicle gloabal (" << poseArrayBuffer->vehPose.position.x << ", "
              << poseArrayBuffer->vehPose.position.y << ")" << std::endl;
...
public:
  PfmsSample(ros::NodeHandle nh) : nh_(nh), it_(nh)
  {
    ...
    poseArrayBuffer.mapSize = 200.0 * 0.1;
    ...
    while (ros::ok())
    {
      ...
      if (imageOK)
      {
        ...
        if (poseArrayBuffer.send)
        {
          ...
          cv::putText(rgbImage, "Sent Path.", cv::Point(30, 30), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.8,
                      CV_RGB(200, 200, 250), 1, CV_AA);
