             ...
             a4_setup::RequestGoal::Response &res)
{
  ...
  if (imageDataBuffer_.imageDeq.size())
  {
  ...
  if (poseDataBuffer_.poseDeq.size())
  {
    pose = poseDataBuffer_.poseDeq.front();

    if (!image.empty())
    {
      res.ack = req.pose.x - pose.position.x <= image.cols*resolution_/2 && req.pose.x - pose.position.x >= -image.cols*resolution_/2 &&
                req.pose.y - pose.position.y <= image.cols*resolution_/2 && req.pose.y - pose.position.y >= -image.cols*resolution_/2; 
    }
  }
    // ### UNLOCK
    ...
    */
    ...
    while (ros::ok())     {
        ...
        if(!image.empty()){
            ...
            const int py = -(8-y)/resolution_ + image.rows/2;
            const int px = (8-x)/resolution_ + image.cols/2;
