...
struct GlobalOrd
{
  ...
  {
    return ((std::abs(this->x - o1.x) < float_comparison_tolerence) &&
            (std::abs(this->y - o1.y) < float_comparison_tolerence));
  }
};
...
struct AircraftContainer
{ /*!< A thread safe container for the Aircraft type */
